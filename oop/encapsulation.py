...
# 2) сокрытие данных (ограничние достура к аттрибутам)
# """
"""
1) публичные аттрибуты/методы наследуются и их можно использовать где угодно
2) защищенные аттрибуты/методы наследуются, но работать с ними можно только внутри класса где аттрибут/метод, и в его подклассах
3) приватные аттрибуты/методы не наследуются, и работать с ними принято только внутри класса, где аттрибут/метол был создан
"""
# #  Виды доступа к аттрибутам
# # 1) public - публичные аттрибуты
# # 2) protected (защищенные) - с одним underscore начале
...
# # print(obj.__attr3) # AttributeError
...
# "=============================Getters/Setters=========================="
# # методы, с помощью которых можно получать/изменять значения атттрибута
...
# (obj2.set_age(20))
# print(obj2.get_age())
# obj2.set_age(2352552) # Возраст не может быть меньше 0 или больше 100
...
class Person:
    ...
    # декоратор property делает из метода в аттрибут
    @property
    def age(self):
        return self.__age
